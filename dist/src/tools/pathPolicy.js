...
export function loadGitignorePatterns(workspaceRoot) {
 ...
 */
export function isDeclarationInNodeModules(relativePath) {
    const normalized = stripLeadingSlash(toPosixPath(relativePath));
    if (!normalized.endsWith(".d.ts"))
        return false;
    const parts = normalized.split("/");
    if (parts.some((part) => part === ".git" || part === ".env"))
        return false;
    return parts.includes("node_modules");
}
export function isIgnoredPath(relativePath, gitignorePatterns = []) {
    const normalized = stripLeadingSlash(toPosixPath(relativePath));
    if (!normalized)
        return false;
    if (isDeclarationInNodeModules(normalized))
        return false;
    const parts = normalized.split("/");
...
export function assertReadablePath(workspaceRoot, targetPath) {
...
export function globIgnorePatterns(workspaceRoot) {
    return [
        "**/.git/**",
        "**/node_modules/**",
        "**/dist/**",
