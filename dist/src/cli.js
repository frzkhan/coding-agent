...
export function parseSessionCommand(input) {
    ...
    if (maxMatch)
        return { type: "max", value: Number(maxMatch[1]) };
...
function printSessionConfig(session) {
    console.log(chalk.cyan("\nSession config"));
...
export function resolveCliModes(params) {
    const shouldPromptInteractively = (params.interactiveOption || !params.rawTask) && params.canUseTty;
    const shouldUseChat = params.chatOption || (!params.rawTask && params.canUseTty);
    return { shouldPromptInteractively, shouldUseChat };
}
export function formatChatHeader(state) {
    return [
...
async function runChatSession(state) {
    ...
    while (true) {
        ...
        console.log(chalk.dim(`\nStop reason: ${result.stopReason} in ${result.steps} step(s) | ${formatTokenUsage(result.tokenUsage)} | dry-run: ${state.dryRun}\n`));
    }
}
export async function runCli(argv) {
    ...
    program.parse(argv);
    const rawTask = program.args.join(" ").trim();
    ...
    try {
        const config = loadConfig();
        const maxSteps = Number.isFinite(opts.maxSteps) ? opts.maxSteps : config.maxSteps;
        ...
        console.log(result.finalResponse);
        console.log(chalk.dim(`\nStop reason: ${result.stopReason} in ${result.steps} step(s) | ${formatTokenUsage(result.tokenUsage)} | dry-run: ${dryRun}`));
